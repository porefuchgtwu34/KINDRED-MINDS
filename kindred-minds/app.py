...
First run creates kindred.db automatically and seeds an admin account:
    username: admin
...
QUIZ_RESULTS = {
    "secure": {
        ...
        "text": "You tend to feel comfortable with closeness and independence alike. You "
                "communicate needs directly, trust reasonably, and see conflict as workable "
                "rather than threatening. Keep nurturing relationships where that security is "
                "met in kind.",
    },
    "anxious": {
        "title": "Anxious Attachment",
        "text": "You care deeply and crave closeness, sometimes worrying about where you stand. "
                "Reassurance helps, but the sturdiest relationships grow when you also build "
                "self-soothing habits so your peace isn't only borrowed from someone else's reply.",
    },
    "avoidant": {
        "title": "Avoidant Attachment",
        "text": "You value independence and may pull back when things get intense. That's not "
                "coldness, it's a strategy that once kept you safe. Practising small moments of "
                "vulnerability, on your own timeline, can deepen connection without it feeling "
...
def init_db():
    ...
    # Seed an admin account if none exists yet
    ...
    if not existing:
        db.execute(
            ...
            (
                "admin",
                "admin@kindredminds.local",
                generate_password_hash("admin123"),
                "Community steward. Reach out any time.",
...
def forgot_password():
    """
    Since this demo has no outbound email server, the reset link is shown
    directly on screen after submitting (clearly labelled as a demo
    behaviour). In production you would email this link instead.
    """
    if request.method == "POST":
        email = request.form.get("email", "").strip().lower()
        db = get_db()
...
# --------------------------------------------------------------------------
# Community feed (public posts + comments)
# --------------------------------------------------------------------------
@app.route("/feed")
@login_required
def feed():
    ...
    return render_template(
        "feed.html", posts=posts, page=page, total_pages=total_pages, category=category
    )
...
# --------------------------------------------------------------------------
# Private messaging (polling-based near-real-time)
# --------------------------------------------------------------------------
@app.route("/messages")
@login_required
def messages_inbox():
...
def poll_messages(username):
    """Returns messages newer than `since` (an ISO timestamp) for near-real-time updates."""
    db = get_db()
    other = db.execute("SELECT * FROM users WHERE username = ?", (username,)).fetchone()
    if not other:
        return jsonify({"error": "User not found"}), 404
...
def admin_dashboard():
    ...
    # Admin inbox: everyone who has ever messaged the admin
    ...
    return render_template(
        "admin.html",
        stats=stats, users=users, page=page, total_pages=total_pages,
